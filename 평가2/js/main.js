...
const heartEl = document.querySelectorAll(".fa-regular.fa-heart");
const formEl = document.querySelectorAll("form");
const inputEl = document.querySelectorAll("input");
const bottomEl = document.querySelectorAll(".bottom_wrap");
const hEls = document.querySelectorAll("h3");
...
localStorage.setItem("comment", `${inputEl}`);

for (let i = 0; i < bottomEl.length; i++) {
  bottomEl[i].addEventListener("submit", (e) => {
    e.preventDefault();

    const inputValue = inputEl[i].value;

    if (inputValue === "") {
      alert("댓글을 입력해주세요");
    } else {
      const newComment = document.createElement("p");
      newComment.innerText = `댓글 : ${inputValue}`;
      hEls[i].appendChild(newComment);
      inputEl[i].value = "";
    }
  });
}

// bottomEl[1].addEventListener("submit", (e) => {
//   e.preventDefault();
//   const inputValueTwo = inputEl[1].value;
//   hEls[1].innerText = `댓글:${inputValueTwo}`;
//   if (inputValueTwo === "") {
//     alert("댓글을 입력해주세요");
//   }
// });

// bottomEl[2].addEventListener("submit", (e) => {
//   e.preventDefault();
//   const inputValueThree = inputEl[2].value;
//   hEls[2].innerText = `댓글:${inputValueThree}`;
//   if (inputValueThree === "") {
//     alert("댓글을 입력해주세요");
//   }
// });
